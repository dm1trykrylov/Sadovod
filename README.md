...
*Подробное описание и презентация к защите проекта находятся в папке [``presentation``](./Presentation)*.
...
* отправляется запрос к API сайта-поставщика прогноза

**Календарь** отображает текущий месяц, каждая дата выделена цветом, соответствующим уровню благоприятности дня для садовых работ. Изначально все даты отмечены благоприятными. Особые планируется получать так:
